$(document).ready(function () {
    function onload() {
        $.ajax({
            type: "GET",
            url: "?controller=Product&function=get_completeorder",
            datatype: "json",
            success: function (data) {
                ...
                if (typeof obj === "object") {
                    var len = obj.length;
                    // console.log(obj)
                    var mytable = $('#complete_order_table').DataTable();
                    mytable.clear().draw();
                    for (var i = 0; i < len; i++) {
                        mytable.row.add($(`
                        <tr>
                        ...
                        <td><button type="button" class="btn btn-rounded btn-primary mark_complete" disabled>Completed</button></td>
                    </tr>
                     `)).draw();
                    }
                }
